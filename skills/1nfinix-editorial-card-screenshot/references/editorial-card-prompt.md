...
如果卡片包含中文，必须显式写出本地 CJK fallback 字体栈，避免截图时 Google Fonts 不可用导致中文乱码、豆腐块或错误替代字体：

```css
body {
  font-family: "Inter", "Noto Sans SC", "Noto Sans CJK SC", "Source Han Sans SC", "Microsoft YaHei", "PingFang SC", sans-serif;
}
...
- 标题必须有足够呼吸感，行高不能过于拥挤。
- 中文长标题要优先做“横向吃宽 + 平衡换行 + 轻微降级字号”，不要硬顶着超大字号直到右侧被裁掉。
...
- 中文右栏的单行视觉长度通常更大，列表、标签、强调词必须允许换行；必要时缩短该栏句长，而不是让文本顶到边界后被裁切。
...
- 如果截图链路经过 `print-to-pdf`，必须补 `@page { size: <canvas-width> <canvas-height>; margin: 0; }`，否则 Chromium 可能在导出阶段悄悄缩放或裁掉右边缘。
...
- 截图态 HTML 中，标题、正文、caption、列表项都应显式允许 `overflow-wrap: anywhere` 或等价策略；网格子项与信息块默认加 `min-width: 0`，避免 Chromium 在窄栏里把长中文短语直接裁掉。
- 对中文卡片，fallback 必须明确包含本地 CJK families，如 `Noto Sans CJK SC` / `Noto Serif CJK SC` / `Source Han Sans SC` / `Microsoft YaHei` / `PingFang SC` / `Songti SC`；不要只写抽象的 `serif` / `sans-serif`。
